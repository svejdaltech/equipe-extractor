...
- [x] Excel column order made configurable: first shipped as an `EXCEL_COLUMN_ORDER` env var, then replaced entirely (per user request — SSH/.env access isn't always practical mid-event) with an in-app **`/settings`** page backed by a new `Setting` key-value DB table (`app/settings.py`), behind the same Basic Auth as everything else. `generate_excel` now takes `column_order` as a parameter rather than reading env/DB itself, keeping `parser.py` DB-agnostic — `main.py` looks up the setting and passes it through. Takes effect immediately on the next export, no restart needed. 48/48 tests pass.
